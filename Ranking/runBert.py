...
def evaluate(model, X_test, bce_loss, best_result, X_test_preq=None, is_test=False):
    ...
    if is_test:
        ...
        if args.task == "tiangong":
            print("Previous Query Best Result: MAP: %.4f MRR: %.4f NDCG@1: %.4f NDCG@3: %.4f NDCG@5: %.4f NDCG@10: %.4f" % (result_pre[0], result_pre[1], result_pre[2], result_pre[3], result_pre[4], result_pre[5]))
    return best_result
#*=================================================*#
